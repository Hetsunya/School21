...
int main(int argc, char *argv[]) {
  ...
  if (!error && (optind + 1 - (options.f || options.e)) < argc) {
   ...
   printf("Error!");
    // // Вывод всех шаблонов поиска
    // printf("Search Patterns:\n");
    // for (int i = 0; i < count_templates; i++) {
    //     char buffer[MAX_LINE_LENGTH];
    //     regerror(0, &templates[i], buffer, sizeof(buffer));
    //     printf("%s\n", buffer);
    // }
...
int print_matches(regex_t *templates, int count_templates, char *filename) {
  ...
  // while (result && fgets(line, MAX_LINE_LENGTH, f) != NULL) {
...
int read_file_templates(regex_t *templates, int *count_templates, char *filename) {
  int result = 1;
